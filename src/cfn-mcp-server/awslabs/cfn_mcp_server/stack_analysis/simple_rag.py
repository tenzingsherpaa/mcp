...
class SimpleRAG:
    ...
    def _create_summary_document(self, scan_data: Dict, region: str, scan_id: str) -> Dict:
        ...
        # Get counts either from direct summary or from overall_summary if available
        ...
        # For the new format, add a standard insight based on unmanaged percentage
        if total_count > 0:
            unmanaged_pct = (unmanaged_count / total_count) * 100
            if unmanaged_pct > 50:
                text += f'Key insight: High proportion of unmanaged resources ({unmanaged_pct:.1f}%).'
            elif unmanaged_pct > 20:
                text += f'Key insight: Moderate proportion of unmanaged resources ({unmanaged_pct:.1f}%).'
    ...
    def _create_critical_analysis_documents(
        ...
        # Handle the new structure or fall back to previous structure
        if 'critical_analysis' in scan_data:
            critical_analysis = scan_data.get('critical_analysis', {})
            detailed_analysis = critical_analysis.get('detailed_analysis', {})
        ...
        # For new structure without explicit critical_analysis section,
        # identify potential critical resources from the resources list
        elif 'resources' in scan_data and 'summary' in scan_data:
            resources = scan_data.get('resources', [])
            
            # Focus on unmanaged resources as potentially critical
            for resource in resources:
                if not resource.get('ManagedByStack', False):
                    doc = self._create_resource_document(resource, region, scan_id)
                    doc['id'] = f'{doc["id"]}-critical'
                    doc['text'] += ' This resource requires attention as it is unmanaged.'
                    documents.append(doc)

...
class TestClient:
    ...
    @patch('awslabs.cfn_stack_analysis_mcp_server.aws_client.session')
    @patch('awslabs.cfn_stack_analysis_mcp_server.aws_client.environ')
    async def test_happy_path(self, mock_environ, mock_session):
        """Testing happy path."""
        client = {}
        mock_session.client.return_value = client

        result = get_aws_client('cloudformation', 'us-east-1')
    ...
    async def test_happy_path_no_region(self, mock_environ, mock_session):
        ...
        result = get_aws_client('cloudformation')
